...
int bloom_lookup(struct bloom * this, char * item) {
	int lookup_value = hash_string(item, BLOOM_HASH1, this->size);
	int lookup_value_two = hash_string(item, BLOOM_HASH2, this->size);
	int result = bitset_lookup(this->bitset, lookup_value);
	int result_two = bitset_lookup(this->bitset, lookup_value_two);
	if((result && result_two) == 1)
	{
		return 1;
	}
	return 0;
...
void bloom_add(struct bloom * this, char * item) {
	int added_value = hash_string(item, BLOOM_HASH1, this->size);
	int added_value_two = hash_string(item, BLOOM_HASH2, this->size);
	bitset_add(this->bitset, added_value);
	bitset_add(this->bitset, added_value_two);
}
